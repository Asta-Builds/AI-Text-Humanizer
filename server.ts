...
function analyzeText(text: string) {
  ...
  if (sentences.length > 1) {
    ...
    // Human prose varies heavily in sentence length (burstiness varies 65-95).
    // Stiff AI output uses sentences of almost identical counts (burstiness stays 15-35).
  ...
  // Word complexity / frequency measure for Perplexity calculation
  ...
  // Calculate weighted score between 10 and 99
  let score = Math.round((density * 45) + (passiveDensity * 25) + uniformPenalty + 15);
  score = Math.max(10, Math.min(99, score));

  // Determine an approximate grade level
...
- Academic Integrity / Plagiarism Safeguard: ${plagiarismSafeguard ? "Yes, ensure critical statistics, names, citations, formulas, and exact double-quotes are 100% untouched." : "Standard preservation"}`;
      ...
      // Analyze the output humanized text
      const humanizedAnalysis = analyzeText(humanizedText);

      // Make sure the "after" score represents a beautiful reduction but doesn't drop to 0.
      // Humans still use normal punctuation and basic transition verbs.
      ...
      // Build comparative sentence alignment
      ...
      // Boost analytics for humanized output
      const result = {
        ...
        metrics: {
          ...
          sentencePairs: sentencePairs.slice(0, 16) // Limit to top 16 sentences for visual display efficiency
        }
      };
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`AI Text Humanizer backend running on http://localhost:${PORT}`);
  });
}
